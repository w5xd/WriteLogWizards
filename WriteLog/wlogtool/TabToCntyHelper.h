...
** If this class detects that situation, it commands WriteLog to set its cursor to the CNTY column, but empty */
    public:
        TabToCntyHelper(unsigned long n, IWriteLog* Parent, int Flags) : n(n), Parent(Parent), Flags(Flags)
        {}
        TabToCntyHelper(IWriteLog* Parent, int Flags) : Parent(Parent), n(0), Flags(Flags)
        { if (Parent && 0 != (Flags & WLOG_MULTICHECK_FILLFPRIOR)) Parent->NumberQsos(&n);  }
        typedef std::function<void(QsoPtr_t, IDispatch*)> OnUpdateQsoFcn;
#pragma warning( push )
#pragma warning( disable : 4996 )
        void tab(QsoPtr_t q, short cntyField, const OnUpdateQsoFcn &f = OnUpdateQsoFcn())
        {
            if (0 == (Flags & WLOG_MULTICHECK_FILLFPRIOR))
                return;
            if (0 != (Flags & WLOG_MULTICHECK_NOWRT))
                return;
