...
for each expression.*/$endif$// Declaration of the $MM_ROVERDLG_CLASS_NAME$
#pragma once
...
// $MM_ROVERDLG_CLASS_NAME$

class $MM_ROVERDLG_CLASS_NAME$ : 
	public CAxDialogImpl<$MM_ROVERDLG_CLASS_NAME$>
{
public:
	typedef std::function<void($MM_ROVERDLG_CLASS_NAME$ &)> InitDialogFcn_t;
	$MM_ROVERDLG_CLASS_NAME$ (const InitDialogFcn_t& f) : m_FocusState(0)
		, m_OnInit(f)
...
$endif$
	enum { IDD = $IDD_ROVERSELECT_DIALOGID$ };
	enum { FOCUS_STATE_NONE, FOCUS_STATE_ADD, FOCUS_STATE_EXISTING };

BEGIN_MSG_MAP($MM_ROVERDLG_CLASS_NAME$)
...
$endif$
	CHAIN_MSG_MAP(CAxDialogImpl<$MM_ROVERDLG_CLASS_NAME$>)
END_MSG_MAP()
...
//  LRESULT NotifyHandler(int idCtrl, LPNMHDR pnmh, BOOL& bHandled);
	...
	{
		CAxDialogImpl<$MM_ROVERDLG_CLASS_NAME$>::OnInitDialog(uMsg, wParam, lParam, bHandled);
		if (m_OnInit)
			m_OnInit(*this);
...
$if$ ($MULTIPLE_NAMED_IN_QSO_TX$ == 1)	std::string GetListTextM(unsigned idc)
	{
		std::string ret;
		auto list = GetDlgItem(idc);
...
$endif$$if$ ($MULTIPLE_NAMED_IN_QSO_TX$ == 0)	std::string GetListTextM(unsigned idc) {return GetListText(idc);}
$endif$
	void OnCreateNew()
	{
